...
class VerifyViewController: UIViewController {
    ...
    func apiVerify(token: String, callback: @escaping (String, String) -> Void, error errorCallback: @escaping (String) -> Void) {
        var request = URLRequest(url: URL(string: "https://api.topcoin.network/origin/api/v1/users/self")!)
    ...
    func success(token: String, email: String) {
        ...
        if let token = UserDefaults.standard.string(forKey: "token") {
            ...
            if let email = UserDefaults.standard.string(forKey: "email") {
                ...
                DispatchQueue.main.asyncAfter(deadline: start + 2, execute: {
                    self.success(token: token, email: email)
                })
            } else {
                apiVerify(token: token, callback: { email, token in
                    DispatchQueue.main.asyncAfter(deadline: start + 2, execute: {
                        self.success(token: token, email: email)
                    })
                }, error: { message in
